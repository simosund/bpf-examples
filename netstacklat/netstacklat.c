...
static const char *hook_to_description(enum netstacklat_hook hook)
{
	...
	case NETSTACKLAT_HOOK_CONNTRACK:
		return "packet has passed through conntrack (on local in)";
	case NETSTACKLAT_HOOK_TCP_START:
	...
	case NETSTACKLAT_HOOK_TCP_SOCK_QUEUE:
		return "packet has been queued to a TCP socket, i.e. end of the kernel receive stack";
	case NETSTACKLAT_HOOK_UDP_SOCK_QUEUE:
		return "packed has been queued to a UDP socket, i.e. end of the kernel receive stack";
	case NETSTACKLAT_HOOK_TCP_SOCK_READ:
			  ...
			  const struct netstacklat_bpf *obj)
{
	...
	case NETSTACKLAT_HOOK_IP_RCV:
		...
		break;
	case NETSTACKLAT_HOOK_CONNTRACK:
		progs->progs[0] = obj->progs.netstacklat_ipv4_conntrack_in;
		progs->progs[1] = obj->progs.netstacklat_ipv6_conntrack_in;
		progs->nprogs = 2;
		break;
	case NETSTACKLAT_HOOK_TCP_START:
