...
static void tcp_update_ooo_range(struct sock *sk)
{
	...
	// out-of-order queue empty - nothing to update
	if (BPF_CORE_READ(tp, out_of_order_queue.rb_node) == NULL)
		return;

	/* Perhaps a bit unnecessary to read in the whole skb->cb
	 * However can't think of a better way to do it in a
	 * CO-RE compliant manner as we also need to cast the cb
	 * Just reading in a pointer to the cb would require an
	 * additional probe-read to read the end_seq.
	 */
	err = BPF_CORE_READ_INTO(&cb, tp, ooo_last_skb, cb);
	if (err) {
		/* Shouldn't happen unless we have some CO-RE issue */
	...
	if (tp_ooo_range->active) {
		if (u32_lt(tp_ooo_range->ooo_seq_end, cb.end_seq))
			tp_ooo_range->ooo_seq_end = cb.end_seq;
	} else {
		tp_ooo_range->ooo_seq_end = cb.end_seq;
