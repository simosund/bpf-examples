...
struct pping_config {
	...
	__u8 ipv4_prefix_size;
	__u8 ipv6_prefix_size;
	bool force;
...
static void generate_ippfrefix_bitmask(struct in6_addr *bitmask, __u8 prefix_size)
{
	int bytes = prefix_size / 8;
	int bits = prefix_size % 8;

	memset(bitmask, 0, sizeof(*bitmask));
	memset(bitmask, 0xff, bytes);
	bitmask->s6_addr[bytes] = (__u8)0xff << (8 - bits);
}
...
static int parse_arguments(int argc, char *argv[], struct pping_config *config)
{
	...
	while ((opt = getopt_long(argc, argv, "hflTCsei:r:R:t:c:F:I:x:a:4:6:",
		...
		case 'r':
			...
			config->bpf_config.rate_limit = user_val * NS_PER_MS;
			break;
		case 'R':
			err = parse_bounded_double(&user_val, optarg, 0, 10000,
						   "rtt-rate");
			if (err)
				return -EINVAL;
			config->bpf_config.rtt_rate =
				DOUBLE_TO_FIXPOINT(user_val);
			break;
		case 't':
		...
		case 'a':
			...
			break;
		case '4':
			err = parse_bounded_double(&user_val, optarg, 0, 32,
						   "aggregate-subnets-v4");
			if (err)
				return -EINVAL;
			config->ipv4_prefix_size = user_val + 96; // +96 due to IPv4 being mapped in IPv6 space
			break;
		case '6':
			err = parse_bounded_double(&user_val, optarg, 0, 128,
						   "aggregate-subnets-v6");
			if (err)
				return -EINVAL;
			config->ipv6_prefix_size = user_val;
			break;
		case 'e':
	...
	generate_ippfrefix_bitmask(&config->bpf_config.ipv4_prefix_mask,
				   config->ipv4_prefix_size);
	generate_ippfrefix_bitmask(&config->bpf_config.ipv6_prefix_mask,
				   config->ipv6_prefix_size);
...
static int format_ipprefix(char *buf, size_t size, int af,
			   struct in6_addr *ip_prefix, __u8 prefix_len)
{
	size_t iplen;
	int err;
	...
	if (af == AF_UNSPEC)
		af = is_ipv4_in_ipv6(ip_prefix) ? AF_INET : AF_INET6;
	if (af == AF_INET) {
		if (prefix_len < 96)
			return -EINVAL;
		prefix_len -= 96;
	...
	format_ip_address(buf, size, af, ip_prefix);

	iplen = strlen(buf);
	err = snprintf(buf + iplen, size - iplen, "/%u", prefix_len);
	if (err > size - iplen)
		err = -ENOSPC;
	else if (err > 0)
		err = 0;
				   ...
				   struct aggregated_rtt_stats *rtt_stats)
{
	...
	fprintf(stream,
		": %s -> count=%llu, min=%llu.%06llu ms, mean=%.2f ms, median=%.2f ms, p95=%.2f ms, max=%llu.%06llu ms",
		prefixstr, lhist_count(rtt_stats->bins, RTT_AGG_NR_BINS),
		rtt_stats->min / NS_PER_MS, rtt_stats->min % NS_PER_MS,
		lhist_mean(rtt_stats->bins, RTT_AGG_NR_BINS, RTT_AGG_BIN_WIDTH,
			   0) /
			NS_PER_MS,
		lhist_percentile(rtt_stats->bins, 50, RTT_AGG_NR_BINS,
				 RTT_AGG_BIN_WIDTH, 0) /
			NS_PER_MS,
		lhist_percentile(rtt_stats->bins, 95, RTT_AGG_NR_BINS,
				 RTT_AGG_BIN_WIDTH, 0) /
			NS_PER_MS,
			       ...
			       struct aggregated_rtt_stats *rtt_stats)
{
	...
	jsonw_float_field(ctx, "mean_rtt",
			  lhist_mean(rtt_stats->bins, RTT_AGG_NR_BINS,
				     RTT_AGG_BIN_WIDTH, 0));
	...
	jsonw_u64_field(ctx, "bin_width", RTT_AGG_BIN_WIDTH);

	jsonw_name(ctx, "histogram");
	jsonw_start_array(ctx);
	for (i = 0; i < RTT_AGG_NR_BINS; i++)
...
static void print_aggregated_rtts(enum PPING_OUTPUT_FORMAT format, __u64 t,
				  struct in6_addr *ip_prefix,
				  __u8 ipv4_prefix_len, __u8 ipv6_prefix_len,
				  struct aggregated_rtt_stats *rtt_stats)
{
	...
	af = is_ipv4_in_ipv6(ip_prefix) ? AF_INET : AF_INET6;
	format_ipprefix(prefixstr, sizeof(prefixstr), af, ip_prefix,
			af == AF_INET ? ipv4_prefix_len : ipv6_prefix_len);

	if (format == PPING_OUTPUT_STANDARD)
		print_aggrtts_standard(stdout, t, prefixstr, rtt_stats);
	else
		print_aggrtts_json(json_ctx, t, prefixstr, rtt_stats);
}
			    ...
			    int n_cpus)
{
	...
	for (i = 0; i < n_cpus; i++) {
		...
		for (bin = 0; bin < RTT_AGG_NR_BINS; bin++)
...
static int report_aggregated_rtts(enum PPING_OUTPUT_FORMAT report_format,
				  int map_active_fd, int map_fd1, int map_fd2,
				  __u8 ipv4_prefix_len, __u8 ipv6_prefix_len)
{
	struct aggregated_rtt_stats *percpu_stats = NULL, *empty_stats = NULL;
	struct aggregated_rtt_stats merged_stats;
	int n_cpus = libbpf_num_possible_cpus();
	struct in6_addr *cur_key = NULL;
	struct in6_addr next_key;
	int agg_map, err;
	__u64 t;
	...
	if (agg_map < 0) {
		err = agg_map;
		goto exit_post_emptystats;
	}

	while ((err = bpf_map_get_next_key(agg_map, cur_key, &next_key)) == 0) {
		err = bpf_map_lookup_elem(agg_map, &next_key, percpu_stats);
		if (err)
			break;
		...
		if (!aggregated_rtt_stats_empty(&merged_stats)) {
			print_aggregated_rtts(report_format, t, &next_key,
					      ipv4_prefix_len, ipv6_prefix_len,
					      &merged_stats);

			err = bpf_map_update_elem(agg_map, &next_key,
...
static void *periodic_rtt_aggregation(void *args)
{
	...
	argp->err = 0;
	while (keep_running) {
		err = report_aggregated_rtts(argp->format, argp->map_active_fd,
					     argp->map_fd1, argp->map_fd2,
					     argp->ipv4_prefix_len,
					     argp->ipv6_prefix_len);
		if (err) {
			libbpf_strerror(err, buf, sizeof(buf));
				    ...
				    struct pping_config *config)
{
	...
	config->agg_args.ipv4_prefix_len = config->ipv4_prefix_size;
	config->agg_args.ipv6_prefix_len = config->ipv6_prefix_size;
...
int main(int argc, char *argv[])
{
	...
	case PPING_OUTPUT_STANDARD:
		...
		config.agg_args.format = PPING_OUTPUT_STANDARD;
		break;
	case PPING_OUTPUT_JSON:
		print_event_func = print_event_json;
		config.agg_args.format = PPING_OUTPUT_JSON;
		break;
	case PPING_OUTPUT_PPVIZ:
	...
	if (config.bpf_config.agg_rtts)
		fprintf(stderr,
			"Aggregating RTTs in histograms with %lu %lu.%06lu ms wide bins\n",
