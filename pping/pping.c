...
static int parse_arguments(int argc, char *argv[], struct pping_config *config)
{
	...
	while ((opt = getopt_long(argc, argv, "hflTCseui:r:R:t:c:F:I:x:a:4:6:o:w:",
		...
		case 'a':
			 ...
			 * interleave in a strange fashion */
		...
		case '4':
			...
			if (err)
				return -EINVAL;
			config->agg_conf.ipv4_prefix_len = user_val;
			break;
		case '6':
			err = parse_bounded_double(&user_val, optarg, 0, 64,
...
/* Formats IPv4 or IPv6 IP-prefix.
 * ip_prefix is expected to point to a __u32/in_addr for IPv4 and a
 * __u64/in6_addr for IPv6. Note that only prefix-lengths up to 64 are supported
 * for IPv6. */
static int format_ipprefix(char *buf, size_t size, int af,
			   void *ip_prefix, __u8 prefix_len)
{
	...
	if (af == AF_INET) {
		...
		memcpy(&ipv6, ip_prefix, sizeof(__u64));
		memset(&ipv6.s6_addr32[2], 0, sizeof(__u64));
		ip_prefix = &ipv6;
	} else {
		return -EINVAL;
	}

	err = inet_ntop(af, ip_prefix, buf, size) ? 0 : -errno;
	if (err)
		return err;
				   ...
				   struct aggregation_config *agg_conf)
{
	...
	jsonw_u64_field(ctx, "bin_width", agg_conf->bin_width);
	jsonw_u64_field(ctx, "aggregation_interval",
			agg_conf->aggregation_interval);
	jsonw_u64_field(ctx, "timeout_interval", agg_conf->timeout_interval);
	...
	jsonw_bool_field(ctx, "truncate_histograms",
			 agg_conf->truncate_histograms);
...
static void print_aggrtts_standard(FILE *stream, __u64 t, const char *prefixstr,
				   ...
				   struct aggregation_config *agg_conf)
{
	int nb = agg_conf->n_bins, bw = agg_conf->bin_width;
...
static void print_aggregated_rtts(struct output_context *out_ctx, __u64 t,
				  void *ip_prefix, int af, __u8 prefix_len,
				  struct aggregated_rtt_stats *rtt_stats,
				  struct aggregation_config *agg_conf)
{
				     ...
				     struct aggregation_config *agg_conf)
{
	...
	while ((err = bpf_map_get_next_key(map_fd, cur_key, &next_key)) == 0) {
		...
		merge_percpu_aggreated_rtts(percpu_stats, &merged_stats, n_cpus,
					    agg_conf->n_bins);
		// Only print prefixes which have data
		if (!aggregated_rtt_stats_empty(&merged_stats)) {
			print_aggregated_rtts(out_ctx, t_monotonic, &next_key,
					      af, prefix_len, &merged_stats,
					      agg_conf);
				  ...
				  struct aggregation_config *agg_conf)
{
	...
	err = report_aggregated_rtt_map(out_ctx,
					maps->map_v4_backup_fd[map_idx],
				    ...
				    struct pping_config *config)
{
	...
	config->agg_args.out_ctx = &config->out_ctx;
	config->agg_args.agg_conf = &config->agg_conf;
	err = fetch_aggregation_map_fds(obj, &config->agg_args.maps);
	if (err)
		return err;
...
int main(int argc, char *argv[])
{
	...
	pb = perf_buffer__new(bpf_object__find_map_fd_by_name(obj,
			      ...
			      handle_missed_events, &config.out_ctx, NULL);
