...
static int parse_arguments(int argc, char *argv[], struct pping_config *config)
{
	...
	while ((opt = getopt_long(argc, argv, "hflTCsi:r:R:t:c:F:I:x:a:4:6:w:",
		...
		case 'w':
			len = strlen(optarg);
			if (len >= sizeof(config->out_ctx.output_file)) {
				fprintf(stderr, "File name too long\n");
				return -ENAMETOOLONG;
			}

			memcpy(config->out_ctx.output_file, optarg, len);
				     ...
				     struct aggregation_config *agg_conf)
{
	...
	if (del_idx > 0)
		err = bpf_map_delete_batch(map_fd, del_keys, &del_idx, &batch_opts);
		       ...
		       struct aggregation_config *agg_conf)
{
	...
	if (out_ctx->write_to_file) {
		f = fopen(out_ctx->output_file, "ax");
		if (!f)
			return -errno;
...
static int handle_aggregation_timer_expiration(
	int timer_fd, struct output_context *out_ctx,
	struct aggregation_maps *maps, struct aggregation_config *agg_conf)
{
	__u64 timer_exps;
	int ret, err;
